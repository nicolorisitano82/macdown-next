...
| ~~**B2**~~ | **Fatto**: il registro (segnalibro + quello che si sa di ogni documento) e `changes.list`. Misurato sul Drive vero: segnalibro `244`, zero cambiamenti; toccato un file; dallo stesso segnalibro **un** cambiamento, col nome e la versione nuova, e il segnalibro diventa `245` | è la parte che rende la cosa una sincronizzazione invece di un carica-e-scarica |
| ~~**B-iCloud**~~ | **Fatto**: iCloud Drive come servizio, che è una cartella — di suo usa una cartella `MacDownNext` dentro iCloud Drive, che si fa da sola la prima volta, e se ne sceglie un'altra nel pannello di sempre, sempre dentro `~/Library/Mobile Documents`, e da lì elenco, lettura e scrittura **coordinate** (`NSFileCoordinator`), versione = data di modifica più dimensione, e la stessa regola di tutti: una scrittura che troverebbe il file mosso si ferma o si mette accanto | era la strada A della roadmap, ed è arrivata prima di Dropbox perché non ha niente da registrare da nessuna parte |
...
| ~~**B4**~~ | **Fatto**: un salvataggio che trova la versione mossa non scrive niente e chiede — **Confronta… · Tieni entrambi · Tieni la mia · Tieni la loro**. «Confronta» apre le due colonne (la tua a sinistra, la loro a destra) e **in fondo alla finestra le stesse tre decisioni sono tre bottoni**: si possono prima portare dentro le righe che si vogliono, riga per riga, e poi mandare su la versione così ottenuta | quando ci sono conflitti veri da mostrare, e non prima |
